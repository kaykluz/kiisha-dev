 ...
 * Llama LLM Adapter
 * 
 * Uses the Together AI API for Llama models.
 * Together AI provides access to Meta's Llama models via an OpenAI-compatible API.
 */
...
export class LlamaLLMAdapter implements LLMProviderAdapter {
  readonly providerId = 'llama' as const;
  ...
  async initialize(config: Record<string, unknown>, secrets?: Record<string, string>): Promise<void> {
    this.config = {
      model: config.model as string || 'meta-llama/Llama-3.3-70B-Instruct-Turbo',
    };
  ...
  async testConnection(): Promise<TestConnectionResult> {
    if (!this.apiKey) {
      return { success: false, message: 'Together AI API key not configured' };
    }
    ...
    try {
      ...
      if (response.choices.length > 0) {
        return {
          success: true,
          message: 'Llama (Together AI) connection successful',
        ...
        return {
          success: false,
          message: 'No response from Llama',
          latencyMs: Date.now() - start,
        };
      }
    } catch (error) {
  ...
  async chat(messages: LLMMessage[], options?: LLMChatOptions): Promise<LLMChatResponse> {
    ...
    const body: Record<string, unknown> = {
      model: options?.model || this.config.model || 'meta-llama/Llama-3.3-70B-Instruct-Turbo',
    ...
    if (!response.ok) {
      ...
      throw new Error(`Llama error: ${error.error?.message || response.status}`);
    }
